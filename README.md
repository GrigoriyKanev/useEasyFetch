...
- Функция логирует ошибки в консоль. При публикации в продакшн можно заменить поведение логирования или выбрасывать исключения в зависимости от потребностей.
- В серверных средах (Node.js) убедитесь, что глобально доступен `fetch` или используйте полифил (например, `node-fetch` для старых версий).

## Публикация на npm

1. Обновите поле `name` в `package.json` на уникальное имя (например, `use-easy-fetch`).
2. Увеличьте `version` согласно semver.
3. Войдите в npm: `npm login`.
4. Публикуйте: `npm publish --access public` (если пакет публичный).
